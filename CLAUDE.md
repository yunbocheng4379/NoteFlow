...
- **Database**: MySQL (see above). A stray SQLite file may exist at `backend/noteflow.db` from earlier versions/migrations — not the active store when `DATABASE_URL` points at MySQL.
