...
export default function ShareCollectionDialog({ collectionId, open, onOpenChange }: Props) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    getCollectionShareStatus(collectionId)
      .then(setStatus)
  ...
  const shareUrl = status?.share_token
    ? `${window.location.origin}/sc/${status.share_token}`
    : ''
  ...
  const handleToggle = async () => {
    ...
    try {
      if (status?.is_active) {
        await disableCollectionShare(collectionId)
        setStatus(s => s ? { ...s, is_active: false } : s)
        toast.success('分享已关闭')
      } else {
        const next = await enableCollectionShare(collectionId)
        setStatus(next)
        toast.success('分享已开启')
      }
    } catch {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="sr-only">管理合集的公开分享链接</DialogDescription>
        </DialogHeader>
