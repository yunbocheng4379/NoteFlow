...
**Goal:** 让 NoteFlow 支持邮箱验证码登录、手机验证码登录（在现有密码登录基础上），并在注册成功后 / 老用户下次登录时强制引导绑定手机号。
...
- `NoteFlow_frontend/src/pages/BindPhonePage/index.tsx` — 绑定手机号页面
...
- `NoteFlow_frontend/src/services/auth.ts` — 新增 `sendCode`/`loginByCode`/`bindPhone` 方法 + 错误码映射 + `UserInfo.phone` 字段
- `NoteFlow_frontend/src/pages/AuthPage/index.tsx` — 改造成两层 Tab
- `NoteFlow_frontend/src/App.tsx` — 新增 `/bind-phone` 路由 + 路由守卫检查 `phone`
...
```typescript
// NoteFlow_frontend/src/services/auth.ts
...
Run: `cd NoteFlow_frontend && pnpm exec tsc --noEmit -p tsconfig.app.json 2>&1 | grep -i "services/auth" || echo "no auth.ts errors"`
Expected: `no auth.ts errors`（其他文件在后续任务修改前可能有类型报错，此步骤只关注 `auth.ts` 本身）

- [ ] **Step 3: Commit**

```bash
git add NoteFlow_frontend/src/services/auth.ts
...
**Files:**
- Modify: `NoteFlow_frontend/src/pages/AuthPage/index.tsx`
...
保留原文件的左侧营销面板（`FEATURES`、`AI_TAGS`、左侧 JSX 区块）不变,只重写右侧表单区域和状态逻辑。完整替换 `NoteFlow_frontend/src/pages/AuthPage/index.tsx`：
...
Run: `cd NoteFlow_frontend && pnpm exec tsc --noEmit -p tsconfig.app.json 2>&1 | grep -i "AuthPage" || echo "no AuthPage errors"`
Expected: `no AuthPage errors`

- [ ] **Step 3: Commit**

```bash
git add NoteFlow_frontend/src/pages/AuthPage/index.tsx
...
**Files:**
- Create: `NoteFlow_frontend/src/pages/BindPhonePage/index.tsx`
...
```tsx
// NoteFlow_frontend/src/pages/BindPhonePage/index.tsx
...
在 `NoteFlow_frontend/src/App.tsx` 里做以下改动：
...
Run: `cd NoteFlow_frontend && pnpm exec tsc --noEmit -p tsconfig.app.json 2>&1 | tail -40`
Expected: 无与 `BindPhonePage`、`App.tsx` 相关的类型错误（若有历史遗留错误与本次改动无关，可忽略，但需在总结里说明）

- [ ] **Step 4: 启动前端 dev server 手动验证**

Run: `cd NoteFlow_frontend && pnpm dev`
...
```bash
git add NoteFlow_frontend/src/pages/BindPhonePage/index.tsx NoteFlow_frontend/src/App.tsx
...
- [ ] 前端: `cd NoteFlow_frontend && pnpm lint` 无新增报错
- [ ] 前端: `cd NoteFlow_frontend && pnpm build` 构建成功
