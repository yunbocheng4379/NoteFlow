...
def _price_as_float(value):
    """将数据库 Numeric/Decimal 转为可直接返回给 JSON 客户端的数字。"""
    ...
    try:
        ...
        if model:
            return {
                ...
                "input_price_per_million": _price_as_float(model.input_price_per_million),
                "output_price_per_million": _price_as_float(model.output_price_per_million),
...
def insert_model(provider_id: int, model_name: str, tier: str = "normal", supports_reasoning: int = 0,
                  supports_vision: int = 0, input_price_per_million=None, output_price_per_million=None,
...
def get_models_by_provider(provider_id: int, tier_filter: Optional[list] = None):
    ...
    try:
        ...
        return [{"id": m.id, "model_name": m.model_name, "tier": m.tier,
                 ...
                 "input_price_per_million": _price_as_float(m.input_price_per_million),
                 "output_price_per_million": _price_as_float(m.output_price_per_million)} for m in models]
    finally:
        db.close()
...
def update_model(model_id: int, tier: str, supports_reasoning: int, supports_vision: int,
                 input_price_per_million=None, output_price_per_million=None) -> bool:
        ...
        return [
            {
                ...
                "supports_vision": bool(m.supports_vision),
                "input_price_per_million": _price_as_float(m.input_price_per_million),
                "output_price_per_million": _price_as_float(m.output_price_per_million),
            }
            for m in models
        ]
    finally:
