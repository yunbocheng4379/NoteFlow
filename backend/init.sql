...
CREATE TABLE IF NOT EXISTS `video_tasks` (
  ...
  `custom_title` varchar(200) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '用户手动设置的笔记标题；非空时覆盖自动提取的视频标题',
...
CREATE TABLE IF NOT EXISTS `orders` (
  ...
  `qrcode_url` varchar(512) DEFAULT NULL COMMENT '真实支付渠道返回的二维码内容 (支付宝 qr_code / 微信 code_url)',
  `trade_no` varchar(64) DEFAULT NULL COMMENT '支付渠道侧交易号 (支付宝 trade_no / 微信 transaction_id)',
