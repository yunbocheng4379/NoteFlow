...
onMessage<{ url: string; title?: string }, 'noteflow-start'>('noteflow-start', async ({ data, sender }) => {
...
browser.runtime.onInstalled.addListener(() => {
  console.log('NoteFlow extension installed')

  // 右键菜单：在视频页或视频链接上"用 NoteFlow 总结"
  try {
    browser.contextMenus.create({
      id: 'noteflow-summarize-page',
      title: '用 NoteFlow 总结此视频',
...
browser.contextMenus?.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId !== 'noteflow-summarize-page')
    return
