...
const label = computed(() => platform ? `用 NoteFlow 总结这个${PLATFORM_LABELS[platform]}视频` : '')
...
async function trigger() {
  ...
  toast.value = null
  try {
    const res = await sendMessage('noteflow-start', {
...
<template>
  <div v-if="platform" class="noteflow-fab fixed bottom-24 right-6 z-[2147483647] flex flex-col items-end gap-2 font-sans select-none">
    <div
      v-if="toast"
