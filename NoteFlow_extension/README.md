...
把 NoteFlow 的"视频链接 → Markdown 笔记"能力下沉到浏览器插件。当前为 P1 MVP（仅工具栏 popup）。
...
2. 点"加载已解压的扩展程序"，选 `NoteFlow_extension/extension/` 目录
3. 启动后端：`cd backend && python main.py`（默认 8483）
4. 浏览器开任意支持的视频页（B 站 / YouTube / 抖音 / 快手），点工具栏 NoteFlow 图标
...
桌面 web 端（`NoteFlow_frontend/`）继续负责：供应商/模型管理、转写器配置、笔记历史。
